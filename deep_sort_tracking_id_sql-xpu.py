...
# 原程式有從 torch_utils import select_device，但我們要自訂，所以不再 import
...
# --------------------- Intel XPU 裝置選擇函式 ---------------------
def select_xpu_device(device_str: str = 'xpu') -> torch.device:
    """
    若 xpu 可用則使用 xpu，否則使用 CPU
    """
    device_str = device_str.lower()
    # 如果使用者想用 'xpu'，且 torch.xpu.is_available() == True，就用 xpu
    if device_str == 'xpu' and hasattr(torch, 'xpu') and torch.xpu.is_available():
        print(">>> 使用 Intel XPU 進行推論 <<<")
        return torch.device("xpu")
    else:
        print(">>> Intel XPU 不可用，將使用 CPU <<<")
...
dashboard_url = (
    "http://127.0.0.1:3000/d/fe18lsbehefpce/"
    "18d6023f-901b-59bd-aff1-a26fd9ceed32?orgId=1"
    "&showCategory=Panel+options&from=1732941598588&to=1732942198588"
)
...
def draw_boxes(img, bbox, names, identities=None, active_objects=None, offset=(0, 0)):
    ...
    # 設定字體
    font_path = "C:/Windows/Fonts/simsun.ttc"  # 請確保字體存在或替換為您系統中的字體
    try:
        font = ImageFont.truetype(font_path, 20)
    except IOError:
        print(f"字體文件 {font_path} 不存在。請更改 font_path 或確保該字體存在。")
...
def detect(save_img=False):
    ...
    # 初始化
    set_logging()
    # ✅ 使用我們自訂的 select_xpu_device()
    device = select_xpu_device(opt.device)
    
    # ✅ Intel XPU 目前無法使用 half 精度；只有 CUDA 時才 half
    ...
    # ✅ 這裡將 use_cuda 改為 (device.type == 'cuda')，避免 DeepSORT 強制使用 GPU
    deepsort = DeepSort(
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # 使用 half 只有在 cuda 時可行，XPU/CPU 需要 float
        ...
        # -------------------------------------------------------------
        #   這裡做關鍵調整：無論 det 是否空，都呼叫 deepsort.update(...)
        # -------------------------------------------------------------
        for i, det in enumerate(pred):
            ...
            if save_img:
                if p not in video_writers:
                    ...
                    video_writers[p] = cv2.VideoWriter(
            ...
            if len(det):
                ...
                # 如果完全沒有檢測到人，就給空值
                xywhs = torch.zeros((0, 4), device=device)
                confs = torch.zeros(0, device=device)
                clss  = torch.zeros(0, device=device)

            # ★ 不論 det 是否為空，都呼叫 update
            outputs = deepsort.update(
                xywhs.cpu(), confs.cpu(), clss.cpu(), im0
            )

            # ---------------------------------------------------------
            #   取得本幀 tracker 內所有 confirmed 且「本幀 time_since_update=1」的 track
            # ---------------------------------------------------------
            current_ids = set()
            for track in deepsort.tracker.tracks:
                if not track.is_confirmed():
                    continue
                # time_since_update 表示距離最後一次 update 的 frame 數
                # <= 1 表示這個 track 在本幀仍被偵測到
                if track.time_since_update <= 1:
                    tid = track.track_id
                    current_ids.add(tid)
                    if tid not in active_objects:
                        person_id = create_person(int(tid))
                        ...
                        # 若已存在，就把 missing_frames 歸零
                        active_objects[tid]['missing_frames'] = 0

            # ---------------------------------------------------------
            #   檢查哪些 ID 在本幀不在 current_ids，missing_frames++，超過 max_age 就刪除
            # ---------------------------------------------------------
            ids_to_remove = []
            for tid in list(active_objects.keys()):
                if tid not in current_ids:
                    active_objects[tid]['missing_frames'] += 1
                    if active_objects[tid]['missing_frames'] >= max_age:
                        data = active_objects[tid]
            ...
            # ---------------------------------------------------------
            #   根據 outputs（當前畫面上確定要畫的 bbox），做繪製/模糊
            # ---------------------------------------------------------
            if len(outputs) > 0:
                bbox_xyxy = outputs[:, :4]
                ...
                im0 = draw_boxes(
                    im0, bbox_xyxy, names, identities, active_objects
                )
            ...
            print(f"Inference Time: {inference_time * 1000:.1f} ms, "
                  f"NMS Time: {nms_time * 1000:.1f} ms, FPS: {fps_display:.1f}")
            ...
            try:
                ...
                print(f"字體文件 {font_path} 不存在。請確保字體文件存在或更改 font_path。")
            ...
            if (datetime.now() - last_insert_time_60s_datetime) >= timedelta(seconds=60):
                ...
                insert_traffic_stats(
                    location_id, start_time_db, end_time_db,
                    person_count, average_duration, occupancy_rate
                )

                if occupancy_rate >= 100:
                    average_occupancy = occupancy_rate
                    insert_congestion_record(
                        location_id, start_time_db, end_time_db, 
                        average_occupancy
                    )
                if occupancy_rate < 60:
                    average_occupancy = occupancy_rate
                    insert_comfort_record(
                        location_id, start_time_db, end_time_db, 
                        average_occupancy
                    )
            ...
            for tid in active_objects:
                ...
                if duration >= overstay_duration_threshold:
                    if not data.get('notified_over_30', False):
                        message = (
                            "\n\n"
                            f"⚠️ 滯留通知：ID為 {tid} 的人員已在 {location_name} 滯留超過30分鐘！ ⚠️\n\n"
                            "🛠️ 請立即採取以下措施：\n"
                            "-----------\n"
                            "#### 1. 滯留檢測與通報\n"
                            "1. **櫃檯人員觀察滯留**\n"
...
if __name__ == '__main__':
    ...
    # ✅ 預設改為 'xpu'，若環境沒裝 Intel XPU，就會回退 CPU
    parser.add_argument('--device', default='xpu', help='選擇 xpu 或 cpu，預設 xpu')
    ...
    # 詢問是否要調整默認容納人數
    question = str(input("請問是否需要變更每平方米容納人數(1=是, 0=否): "))
    while True:
        if question == '1':
            while True:
                try:
                    limit_the_number_of_people = int(input("請輸入每平方米容納人數(默認值為3): "))
    ...
    # 添加 RTSP 來源 (可自行修改)
    rtsp_url_1 = (
        "rtsp://admin:Mlab03+-@192.168.0.118:554/cam/realmonitor"
        "?channel=1&subtype=0&buffer_size=1&fps=30"
    )
    rtsp_url_2 = (
        "rtsp://admin:Mlab03+-@192.168.0.118:554/cam/realmonitor"
        "?channel=2&subtype=0&buffer_size=1&fps=30"
    )
